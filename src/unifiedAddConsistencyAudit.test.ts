...
const six = ['expense', 'income', 'transfer', 'refund', 'repayment', 'bill'];
...
describe('unified Add consistency follow-up', () => {
  it('keeps Bill as PlannedPayment while preserving the same six-choice launcher shell', () => {
    expect(planned).toContain('mv-unified-add-modal');
    expect(planned).toContain('mv-transaction-type-tabs');
    for (const label of six) expect(planned).toContain(label);
  ...
  it('makes all launcher choices visibly button-like using semantic appearance tokens', () => {
  ...
  it('removes only the redundant payer/recipient helper presentation and leaves transfer logic untouched', () => {
