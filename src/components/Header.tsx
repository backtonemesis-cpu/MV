...
export const Header: React.FC<HeaderProps> = ({
  ...
  const canExportBackup = session?.role === 'owner' || session?.role === 'editor';
    ...
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-neutral-900/95 backdrop-blur border-b border-neutral-200 dark:border-neutral-800 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {canSimulateIdentity && (
              <button
                ...
                title="Run development acceptance checks"
              >
                <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                <span className="hidden md:inline">Development Checks</span>
              </button>
            )}
            ...
            {canSimulateIdentity ? (
              <div className="relative">
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-neutral-900 rounded-xl shadow-xl border border-neutral-200 dark:border-neutral-700 p-2 z-50">
                    <div className="px-3 py-2 border-b border-neutral-100 dark:border-neutral-800 mb-1">
                      <p className="text-xs font-semibold text-neutral-700 dark:text-neutral-200">Development identity simulation</p>
                      <p className="text-[11px] text-neutral-500 dark:text-neutral-400">Unavailable in production.</p>
                    </div>
                    ...
                    <div className="pt-2 mt-2 border-t border-neutral-100 dark:border-neutral-800 px-2">
                      <p className="text-[11px] font-medium text-neutral-600 dark:text-neutral-300 mb-1.5">Test unknown Pending account:</p>
                      <div className="flex gap-1">
                        <input
                          ...
                          className="flex-1 px-2 py-1 text-xs border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100 rounded"
                        />
                        <button
                          onClick={() => {
                            if (customEmail.trim()) {
                          ...
                          className="px-2.5 py-1 text-xs bg-neutral-800 dark:bg-neutral-200 text-white dark:text-neutral-900 rounded font-medium"
                        >
                          Test
                        </button>
                      </div>
              ...
              <div className="flex items-center gap-2">
                <div className="hidden sm:block text-right text-xs">
                  <div className="font-semibold text-neutral-800 dark:text-neutral-200 leading-tight">{session?.name || 'User'}</div>
                  <div className="text-neutral-500 dark:text-neutral-400 text-[10px] truncate max-w-[150px]">{session?.email}</div>
                </div>
                <button
                  type="button"
        ...
        <div className="sm:hidden py-1.5 border-t border-neutral-100 dark:border-neutral-800 flex items-center justify-between text-xs">
          <span className="text-neutral-500 dark:text-neutral-400 text-[11px] truncate max-w-[60%]">{session?.email}</span>
