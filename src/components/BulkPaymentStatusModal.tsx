...
interface BulkPaymentStatusModalProps {
  ...
  accounts: Account[];
  onClose: () => void;
  onMarkPaid: (ids: string[], actualDate: string) => Promise<void>;
  onUndoPaid: (ids: string[]) => Promise<void>;
}
...
export const BulkPaymentStatusModal: React.FC<BulkPaymentStatusModalProps> = ({
  ...
  const totalPence = payments.reduce(
    (sum, payment) =>
      sum + (mode === 'undo' ? payment.actualAmountPence ?? payment.amountPence : payment.amountPence),
    0
  );
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (mode === 'mark') {
        await onMarkPaid(ids, date);
      } else {
        await onUndoPaid(ids);
      }
      onClose();
    } catch (err: any) {
      setError(err.message || (mode === 'mark' ? 'Failed to record payments.' : 'Failed to undo payments.'));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="mv-modal-form">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-lg border border-muted bg-surface-muted px-3 py-2">
              <div className="text-[9px] font-semibold uppercase tracking-wide text-subtle">Bills</div>
              <div className="mt-0.5 text-sm font-bold text-main">{payments.length}</div>
            </div>
            <div className="rounded-lg border border-muted bg-surface-muted px-3 py-2">
              <div className="text-[9px] font-semibold uppercase tracking-wide text-subtle">Total</div>
          ...
          {mode === 'mark' && (
            <div>
              <label htmlFor="bulk-payment-date" className="mb-1 block text-xs font-semibold text-muted">
                Payment date
              </label>
              <input
          ...
          <div className="max-h-56 overflow-y-auto rounded-lg border border-muted">
            {payments.map((payment) => {
              ...
              return (
                <div key={payment.id} className="flex items-start justify-between gap-3 border-b border-muted px-3 py-2 last:border-b-0">
                  <div className="min-w-0">
                    <div className="truncate text-xs font-semibold text-main">{payment.name}</div>
                    <div className="mt-0.5 text-[10px] text-subtle">
                      {account ? accountIdentityLabel(account) : 'Assigned account unavailable'}
                    </div>
                  </div>
                  <div className="mv-private-value shrink-0 font-mono text-xs font-bold tabular-nums text-main">
                    {formatPence(mode === 'undo' ? payment.actualAmountPence ?? payment.amountPence : payment.amountPence)}
                  </div>
                </div>
              );
            })}
          ...
          <div className="mv-modal-actions">
            ...
            >
              {mode === 'mark' ? <CheckCircle2 className="h-3.5 w-3.5" /> : <RotateCcw className="h-3.5 w-3.5" />}
              {saving ? 'Working…' : mode === 'mark' ? 'Mark paid' : 'Undo payments'}
            </button>
          </div>
        </div>
      </section>
